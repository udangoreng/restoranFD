    ...
    <div class="container mt-4">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <span class="badge bg-{{ $reservation->status == 'Confirmed' ? 'success' : ($reservation->status == 'Pending Payment' ? 'warning' : ($reservation->status == 'Created' ? 'info' : ($reservation->status == 'Cancelled' ? 'danger' : ($reservation->status == 'Dine' ? 'info' : ($reservation->status == 'Completed' ? 'success' : 'secondary'))))) }}">
                    {{ $reservation->status }}
                </span>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p><strong>Name:</strong> {{ $reservation->salutation }} {{ $reservation->first_name }} {{ $reservation->last_name }}</p>
                    ...
                    <div class="col-md-6">
                        ...
                        <p><strong>Date:</strong> {{ \Carbon\Carbon::parse($reservation->booking_date)->format('d M Y') }}</p>
                        ...
                        <p><strong>Created:</strong> {{ \Carbon\Carbon::parse($reservation->created_at)->format('d M Y H:i') }}</p>
                    </div>
                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.reservation.update-status', $reservation->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
                            <div class="row g-3 align-items-center">
                                ...
                                <div class="col-auto">
                                    <select name="status" id="status" class="form-select" onchange="showTableAssignment(this)">
                                        ...
                                        <option value="Dine" {{ $reservation->status == 'Dine' ? 'selected' : '' }}>Mark as Dining</option>
                        ...
                        <form action="{{ route('admin.reservation.update-table', $reservation->id) }}" method="POST" class="d-inline ms-3">
                            @csrf
                            @method('PUT')
                            <div class="row g-3 align-items-center">
                                <div class="col-auto">
        ...
        @if($reservation->orders->count() > 0)
            ...
            <div class="card mb-4">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-{{ $order->status == 'Completed' ? 'success' : ($order->status == 'Processing' ? 'info' : ($order->status == 'Pending Payment' ? 'warning' : 'secondary')) }}">
                        {{ $order->status }}
                    </span>
                </div>
                <div class="card-body">
                                ...
                                <div class="card-body">
                                    ...
                                    <table class="table table-borderless">
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-end">Rp {{ number_format($order->down_payment_amount, 0, ',', '.') }}</td>
                                        </tr>
                                        <tr>
                                            <td>Remaining Amount:</td>
                                            <td class="text-end">Rp {{ number_format($order->remaining_amount, 0, ',', '.') }}</td>
                                        </tr>
                                        <tr class="border-top">
                        ...
                        <div class="col-12">
                            <div class="payment-actions">
                                ...
                                <div class="text-center mb-3">
                                    <button type="button" class="btn btn-primary btn-lg" onclick="payFullPayment('{{ $reservation->id }}')">
                                ...
                                <div class="text-center">
                                    <form action="{{ route('admin.reservation.process-payment', $reservation->id) }}" method="POST" class="d-inline">
                                        @csrf
                                        <div class="row g-3 align-items-center justify-content-center">
                                            <div class="col-auto">
                                                <label for="payment_method" class="col-form-label">Or Process Cash Payment:</label>
                                            </div>
                                            <div class="col-auto">
...
<script>
    ...
    function payFullPayment(id) {
        ...
        $('#snap-container').html('<div class="text-center"><div class="spinner-border text-primary" role="status"></div><p>Loading full payment...</p></div>');
                ...
                if (response.status === 'success') {
                    window.snap.pay(response.snapToken, {
                        onSuccess: function(result){
                            window.location.href = '/reservation/' + id + '/payment/callback?order_id=' + result.order_id + '&transaction_status=capture&payment_type=' + result.payment_type;
                        },
